...
<div class='row justify-content-center'>
    <div class="card-header"><span class="card-span">Student Pass Registration</span></div>
    <div class="form-content">
        <form class='col-sm-9' method="post" action="{{ url('/addPassHolder/student')}}" enctype="multipart/form-data">
        <h5 class="alert-danger">{{Session::get('ferror') ?? ''}}</h5> 
            {{ csrf_field() }}
                <div class='form-group row'>
                    <label class="col-md-4 col-form-label text-md-right">Enter Student First Name</label>
                    <input type="text" name='fname' class="form-control col-md-6" value="{{ $input['id'] ?? '' }}" required/>
                </div>
                <div class='form-group row'>
                    <label class="col-md-4 col-form-label text-md-right" >Enter Student Last name</label>
